...
def clean_str(string):
    ...
    """
    string = re.sub(r"[^A-Za-z0-9(),!?\'\`]", " ", string) #Throw away everything except letters and (),!?'`
    ...
    string = re.sub(r"\)", " \) ", string)
    string = re.sub(r"\?", " \? ", string)
    string = re.sub(r"\s{2,}", " ", string)  #Multiple spaces
...
#New function to load test set (polaritydata)
...
#TODO: how to load in word2vec? Read following function:
